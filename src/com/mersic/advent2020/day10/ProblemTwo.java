...
public class ProblemTwo {
    ...
    private static BigInteger countValidAdapterCombinations(List<Integer> adapters, int prev) {
        if (adapters.size() == 1) {
            ...
            BigInteger with = memo.get("with_" + adapters.get(0)+"_prev_"+prev);
            BigInteger withOut = memo.get("without_" + adapters.get(0)+"_prev_"+prev);

            if (with == null) {
                with = countValidAdapterCombinations(adapters.subList(1, adapters.size()), adapters.get(0));
                memo.put("with_" + adapters.get(0)+"_prev_"+prev, with);
            }
            if (withOut == null) {
                withOut = countValidAdapterCombinations(adapters.subList(1, adapters.size()), prev);
                memo.put("without_" + adapters.get(0)+"_prev_"+prev, withOut);
            }
